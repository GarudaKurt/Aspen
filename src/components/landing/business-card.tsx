  ...
  return (
    <article className="group min-w-0 overflow-hidden rounded-xl border border-[#e0e0dd] bg-white shadow-[0_2px_5px_rgba(20,20,20,0.15)] transition-shadow hover:shadow-[0_4px_10px_rgba(20,20,20,0.18)]">
      <div onPointerDown={handlePointerDown} onPointerUp={handlePointerUp} onPointerCancel={() => { pointerStart.current = null; }} className="relative aspect-[1.35] cursor-grab touch-none overflow-hidden bg-[#d5e9e6] active:cursor-grabbing">
        <img draggable={false} src={cardImages[activeImage].src} alt={`Kalinga Animal Hospital view ${activeImage + 1}`} className="block h-[calc(100%-0.5rem)] w-full translate-y-2 select-none object-cover transition-[object-position] duration-300" style={{ objectPosition: cardImages[activeImage].position }} />
        ...
        <button type="button" onClick={() => setSaved((current) => !current)} aria-label={saved ? "Remove from favorites" : "Save Kalinga Animal Hospital"} aria-pressed={saved} className="absolute right-3 top-4 z-10 flex size-10 items-center justify-center rounded-full border border-[#3c6355] bg-white/95 text-[#3c6355] shadow-sm transition-colors"><Heart size={19} fill={saved ? "currentColor" : "none"} strokeWidth={1.8} /></button>
