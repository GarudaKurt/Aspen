    ...
    <section id="browse" className="px-6 pb-12 pt-8 sm:px-10 sm:pb-16 sm:pt-10 lg:px-[130px]">
      <div className="mx-auto max-w-[1440px]">
        ...
        <form onSubmit={submitSearch} className="mt-7 flex max-w-[660px] flex-col overflow-hidden rounded-2xl border border-[#c8c8c5] bg-white sm:h-[62px] sm:flex-row sm:items-center">
          ...
          <Label className="flex min-h-[52px] items-center gap-2 border-t border-[#e6e5e1] px-5 text-[#444743] sm:border-t-0"><MapPin size={18} strokeWidth={1.4} /><select value={city} onChange={(event) => onCityChange(event.target.value)} aria-label="Choose city" className="bg-transparent text-base outline-none">{cities.map((option) => <option key={option}>{option}</option>)}</select></Label>
          <Button variant="ghost" type="submit" className="mx-2 mb-2 inline-flex min-h-[44px] items-center justify-center gap-2 rounded-xl bg-[#3c6355] px-5 text-base font-semibold text-white transition-colors hover:bg-[#2f5044] sm:my-2 sm:mb-0">→ <span>Search</span></Button>
        </form>
