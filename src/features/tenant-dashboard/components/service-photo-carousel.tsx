...
export function ServicePhotoCarousel({
  ...
  return (
    <div
      className="relative h-48 overflow-hidden bg-[#e8f5ef]"
    ...
    >
      {hasPhotos ? (
        ...
        <div className="grid size-full place-items-center text-5xl font-bold text-[#3c6355]">
          {title.charAt(0)}
        </div>
      )}
      {photos.length > 1 && (
          ...
          <div className="absolute bottom-2 left-1/2 flex -translate-x-1/2 gap-1.5 rounded-full bg-black/30 px-2 py-1">
            {photos.map((photo, photoIndex) => (
              <button
                ...
                className={`size-1.5 rounded-full ${photoIndex === index ? "bg-white" : "bg-white/50"}`}
              />
            ))}
          </div>
        </>
