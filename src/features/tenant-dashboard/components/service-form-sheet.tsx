...
const emptyDraft: ServiceDraft = { title: "", category: "Grooming", description: "", price: "", duration: "", photos: [], status: "Draft" };
...
export function ServiceFormSheet({ open, service, onClose, onSave }: { open: boolean; service: ServiceItem | null; onClose: () => void; onSave: (draft: ServiceDraft) => void }) {
  ...
  useEffect(() => {
    if (open) setDraft(service ? { title: service.title, category: service.category, description: service.description, price: service.price, duration: service.duration, photos: service.photos ?? [], status: service.status } : emptyDraft);
    setError("");
    setPhotoError("");
  }, [open, service]);
  ...
  const addPhotos = (files: FileList) => {
    ...
    const invalid = incoming.find((file) => !allowedTypes.has(file.type) || file.size > maxFileSize);
    if (invalid) {
    ...
    setPhotoError("");
    setDraft((current) => ({ ...current, photos: [...current.photos, ...incoming.slice(0, remaining).map((file) => URL.createObjectURL(file))] }));
  };

  const removePhoto = (index: number) => setDraft((current) => ({ ...current, photos: current.photos.filter((_, photoIndex) => photoIndex !== index) }));
  const movePhoto = (index: number, direction: -1 | 1) => setDraft((current) => {
    ...
    <SheetContent title={service ? "Edit service" : "Add service"} onClose={onClose}>
      <form onSubmit={submit} className="flex flex-1 flex-col">
        <div className="space-y-5 p-5">
          ...
          <ServicePhotoPicker photos={draft.photos} error={photoError} onAdd={addPhotos} onRemove={removePhoto} onMove={movePhoto} />
